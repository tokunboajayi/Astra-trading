...
req "onboarding (new, HLX)"       200 POST /api/onboarding '{"experience":"new","symbol":"HLX"}'; has "  tier 1" '"tier":"beginner"'
...
req "quote (a past bar)"           200 GET  "/api/quote?symbol=HLX&as_of=10"
...
req "limit order at Tier 1"        403 POST /api/orders '{"symbol":"HLX","side":"BUY","qty":1,"type":"LIMIT","limit_price":150,"as_of":40}'; has "  locked" 'TIER_LOCKED'
req "stale cursor"                 409 POST /api/orders '{"symbol":"HLX","side":"BUY","qty":1,"as_of":3}'
req "buy 10 HLX"                  200 POST /api/orders '{"symbol":"HLX","side":"BUY","qty":10,"as_of":40}'; has "  filled at 168.00" '"price":168.0'
req "too many shares"              400 POST /api/orders '{"symbol":"HLX","side":"BUY","qty":1000}'; has "  insufficient funds" 'INSUFFICIENT_FUNDS'
...
req "accept the safety net"        200 POST /api/safety-net '{"symbol":"HLX","decision":"accept"}'; has "  stop 151.20" '"stop_price":151.2'
...
echo "stubs & static"
req "news (stub)"                  200 GET  "/api/news?symbol=HLX";                      has "  stub" '"stub":true'
req "fundamentals (stub)"          200 GET  "/api/fundamentals?symbol=HLX";              has "  stub" '"stub":true'
